    ...
    <div className="mt-12 pt-10 border-t border-cream-200">
      <div className="flex items-center justify-between mb-8">
        ...
        {!showForm && (
          <button
            ...
            className="font-sans text-[10px] tracking-[0.2em] uppercase text-bark-400 hover:text-bark-600 border border-cream-300 px-4 py-2 transition-colors"
          >
            Write a Review
          </button>
        )}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-cream-100 border border-cream-300 p-6 mb-8 space-y-4">
          ...
          <div>
            <p className="font-sans text-[10px] tracking-[0.2em] uppercase text-bark-400 mb-2">Your Rating</p>
            <StarRating value={rating} onChange={setRating} />
          </div>
          <div>
            <label className="block font-sans text-[10px] tracking-[0.2em] uppercase text-bark-400 mb-2">Your Name</label>
          ...
          <div>
            <label className="block font-sans text-[10px] tracking-[0.2em] uppercase text-bark-400 mb-2">Your Review</label>
          ...
          {reviews.map(r => (
            <div key={r.id} className="border-b border-cream-200 pb-6 last:border-0">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-sans text-[10px] text-bark-400">{new Date(r.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</span>
              </div>

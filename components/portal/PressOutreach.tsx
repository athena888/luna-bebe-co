...
import { Loader, Sparkles, Bell, Plus, ChevronDown, ChevronUp, Ban, Search } from 'lucide-react'
import type { PressContact } from '@/lib/press-pitch'

// Manual press & gift-guide outreach — rendered inside Morning Review under
// the Press toggle. Emily completes (or AI-crawls) per-contact fields,
// generates Gmail DRAFTS (never sends), and tracks the pipeline. 10/week cap.
...
function needsCrawl(c: PressContact): boolean {
  return c.status === 'new' && !(c.email?.trim() && c.contact_name?.trim() && c.recent_article_url?.trim())
}

export function PressOutreach() {
  ...
  const [crawling, setCrawling] = useState<string | null>(null)   // contact id or 'all'
  ...
  async function crawlOne(id: string): Promise<string> {
    const r = await fetch('/api/portal/press/crawl', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }).then(x => x.json())
    if (r.error) return `error: ${r.error}`
    const f = r.result?.found ?? {}
    return [f.name ? 'name ✓' : 'name —', f.email ? 'email ✓' : 'email —', f.article ? 'article ✓' : 'article —'].join(' ')
  }

  // Sequential crawl (one request per contact so each stays in budget) — same
  // discovery approach as the B2B prospector, published emails only.
  async function crawlAllMissing() {
    const targets = contacts.filter(needsCrawl)
    if (!targets.length) { setNotice('Nothing to crawl — all new contacts have name + email + article.'); return }
    setCrawling('all'); setNotice(null)
    try {
      for (let i = 0; i < targets.length; i++) {
        setCrawlProgress(`Crawling ${i + 1}/${targets.length}: ${targets[i].outlet}…`)
        const summary = await crawlOne(targets[i].id)
        setCrawlProgress(`${targets[i].outlet}: ${summary}`)
      }
      setNotice(`Crawl finished for ${targets.length} outlet(s). Auto-found fields are marked in each row's notes — VERIFY before drafting (especially emails).`)
    } finally {
      setCrawling(null); setCrawlProgress(null)
      await load()
    }
  }

  async function crawlSingle(id: string, outlet: string) {
    setCrawling(id); setNotice(null)
    try {
      const summary = await crawlOne(id)
      setNotice(`${outlet}: ${summary} — verify in the row before drafting.`)
    } finally {
      setCrawling(null)
      await load()
    }
  ...
  const crawlable = contacts.filter(needsCrawl).length
  ...
  return (
    <div>
      ...
      {crawlProgress && <p className="font-sans text-xs text-bark-600 bg-cream-200 border border-cream-300 rounded-lg px-3 py-2 mb-2">{crawlProgress}</p>}
      ...
      <div className="bg-white rounded-2xl border border-cream-300 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <button onClick={crawlAllMissing} disabled={crawling !== null || !crawlable}
              className="inline-flex items-center gap-1 bg-cream-200 hover:bg-cream-300 disabled:opacity-40 text-bark-600 rounded px-2.5 py-1.5 font-sans text-xs">
              {crawling === 'all' ? <Loader size={12} className="animate-spin" /> : <Search size={12} />} Find contacts (AI) {crawlable ? `· ${crawlable}` : ''}
            </button>
            ...
            return (
              <div key={c.id} className={`border rounded-xl px-3 py-2.5 ${c.suppressed ? 'border-rose-300 bg-rose-200/20' : ready ? 'border-sage-200' : 'border-cream-200'}`}>
                <div className="flex items-center gap-2.5">
                  ...
                  {needsCrawl(c) && (
                    <button onClick={() => crawlSingle(c.id, c.outlet)} disabled={crawling !== null}
                      title="Find this outlet's contact with AI web search"
                      className="shrink-0 border border-cream-300 hover:border-bark-400 disabled:opacity-40 rounded px-2 py-1 font-sans text-[10px] uppercase tracking-wide text-bark-500 inline-flex items-center gap-1">
                      {crawling === c.id ? <Loader size={10} className="animate-spin" /> : <Search size={10} />} Find
                    </button>
                  )}

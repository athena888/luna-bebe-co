              ...
              {catalogProducts.map(p => {
                ...
                return (
                  <Link key={p.slug} href={`${isEs ? '/boxes' : '/boxes'}/${p.slug}`} className="group block border border-cream-300 hover:border-bark-400 transition-colors">
                    <div className="relative h-[85vh] bg-cream-100">
                      {cover
                    ...
                    <div className="p-5">
                      <h2 className="font-serif text-xl text-espresso group-hover:text-bark-600">{p.name}</h2>
                      {p.subtitle && <p className="font-serif italic text-sm text-bark-400 mt-0.5">{p.subtitle}</p>}
                      <p className="font-sans text-sm text-bark-600 mt-2">

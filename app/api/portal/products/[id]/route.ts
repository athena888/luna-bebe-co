...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { name, description, price, category, tag, ingredients, inventoryQuantity, hasVariants, certifications, active, needsReview } = body

  // Save core fields first — never let certifications block a save
  try {
    await updateProduct(id, { name, description, price, category, tag, ingredients, hasVariants, active, needsReview })
  } catch {

...
export async function GET(req: NextRequest) {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 3500,
      system: `You write for Petite Lavande's journal — a luxury organic baby & new-mama gift box brand (petitelavande.com). Voice: warm, honest, mother-first, never listicle-clickbait. SEO goal: rank for long-tail queries around baby gifting, postpartum care, organic baby products, baby showers. Hard rules: never the words "wax seal" (say "signature seal"); factual claims only (organic cotton on selected baby textiles, hand-packed in Seattle; never mention GOTS, never call a whole box organic, never make safety or testing claims such as dermatologist tested or safe from birth); include 2-3 natural internal links as markdown [text](path) chosen ONLY from: ${MONEY_LINKS.join(', ')}. Body format: plain text where "## " starts a heading, "- " starts a bullet, blank lines separate paragraphs. 600-900 words.`,
      messages: [{
        role: 'user',

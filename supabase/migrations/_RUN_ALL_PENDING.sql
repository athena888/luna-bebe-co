...
-- 37) Marketing machine shared infrastructure — marketing_contacts (our
--     customer-marketing source of truth, opt-in gated) + campaign attribution
--     on the email log. NOTE: named marketing_contacts because the plain
--     `contacts` table already exists and belongs to the B2B outreach pipeline.
--     (Also in supabase/migrations/marketing_contacts.sql)
...
-- 38) Referral loop (Build 6) — one give-15/get-15 code per paid order.
--     A row is minted at checkout completion (flag-gated); redemption is
--     recorded when a later order pays with that code, and the referrer's
--     $15 thank-you code is tracked here too.
--     (Also in supabase/migrations/referrals.sql)
...
-- 39) Occasion dates (Build 3) — remembered due dates / baby birthdays for
--     perfectly-timed sends. Sends only go to opted-in marketing contacts and
--     the whole flow is OFF until OCCASIONS_ACTIVE=true.
--     (Also in supabase/migrations/occasions.sql)
...
--     (portal one-off sends, logged with attribution).
...
--     table stays as a read fallback until §46 is live.
...
-- 53) Review provenance (Google review feed 2.4). order_id ties a review to
--     the purchase it came from (recovered from the signed review-ask token,
--     never user-typed). collection_method records HOW the review arrived:
...
--     Backfill: NULL collection_method is treated as 'unsolicited' everywhere.
--     Idempotent.

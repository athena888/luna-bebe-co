...
> Rules: EMAIL-DRIVEN ONLY (Emily, 2026-07-27) — the public review form never asks for an email or mentions the reward. The post-delivery review-ask email carries signed ?rt= links (90d, lib/review-token.ts); the form passes the token back silently; the server recovers the buyer's email, re-checks a paid order, and sends the code. ANY rating — never conditioned on sentiment (FTC 16 CFR 465), one reward per email ever, rewarded reviews stamped `incentivized` and excluded from the Google review feed (Google bars incentivized reviews).
> review-ask email adds: "And as a thank-you, every review earns a one-time 20% code for your next box — whatever the rating." / ES: "Y para darte las gracias: cada reseña recibe un código único de 20% de descuento para tu próxima caja, sin importar la calificación." (ES item links → /es/productos/*)
